...
const OrderSummary = ({ totalPrice, items }) => {
  ...
  const handlePlaceOrder = async (e) => {
    ...
    try {
      if (!user) {
        return toast('Sipariş vermek için lütfen giriş yapınız...')
      }
      if (!selectedAddress) {
        return toast('Lütfen bir adres seçiniz...')
      }
      const token = await getToken()
  ...
  return (
    <div className='w-full max-w-lg lg:max-w-[340px] bg-slate-50/30 border border-slate-200 text-slate-500 text-sm rounded-xl p-7'>
      ...
      <p className='text-slate-400 text-xs my-4'>Ödeme Yöntemleri</p>
      ...
      <div className='pb-4 border-b border-slate-200'>
        <div className='flex justify-between'>
          ...
          <div className='flex flex-col gap-1 font-medium text-right'>
            ...
            <p>
              <Protect plan={'plus'} fallback={`${currency} 90`}>
                Ücretsiz
              </Protect>
            </p>
        ...
        {!coupon ? (
          <form
            onSubmit={(e) =>
              toast.promise(handleCouponCode(e), { loading: 'Kupon kontrol ediliyor...' })
            }
          ...
          <div className='w-full flex items-center justify-center gap-2 text-xs mt-2'>
            <p>
              Kod: <span className='font-semibold ml-1'>{coupon.code.toUpperCase()}</span>
            </p>
      ...
      <div className='flex justify-between py-4'>
        ...
        <p className='font-medium text-right'>
          ...
          ${
            coupon
              ? (totalPrice + 90 - (coupon.discount / 100) * totalPrice).toFixed(2)
              : (totalPrice + 90).toLocaleString()
          }`}
          >
            {currency}
            {coupon
      ...
      <button
        onClick={(e) =>
          toast.promise(handlePlaceOrder(e), { loading: 'Siparişiniz hazılanıyor...' })
        }
