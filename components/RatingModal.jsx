...
const RatingModal = ({ ratingModal, setRatingModal }) => {
  ...
  const handleSubmit = async () => {
    if (rating < 0 || rating > 5) {
      return toast('Lütfen oylama için bir puan seçin..')
    }
    if (review.length < 5) {
      return toast('Açıklamanızı buraya yazabilirsiniz..')
    }
  ...
  return (
    <div className='fixed inset-0 z-120 flex items-center justify-center bg-black/10'>
      <div className='bg-white p-8 rounded-lg shadow-lg w-96 relative'>
        ...
        ></textarea>
        <button
          onClick={(e) => toast.promise(handleSubmit(), { loading: 'Gönderlliyor...' })}
