...
export default function AdminCoupons() {
  ...
  return (
    <div className='text-slate-500 mb-40'>
      {/* Add Coupon */}
      <form
        onSubmit={(e) => toast.promise(handleAddCoupon(e), { loading: 'Kupon tanımlanıyor...' })}
        className='max-w-sm text-sm'
      >
        <h2 className='text-2xl'>
          <span className='text-slate-800 font-medium'>Kupon</span> Planlamaları
        </h2>
        ...
        <label>
          <p className='mt-3'>Kupon Son Kullanma Tarihi</p>
          <input
            type='date'
            placeholder='Kupon tarihi'
        ...
        <div className='mt-5'>
          <div className='flex gap-2 mt-3'>
            ...
            <p>Kayıt Teşvik Promosyonu için </p>
          </div>
          <div className='flex gap-2 mt-3'>
            <label className='relative inline-flex items-center cursor-pointer text-gray-900 gap-3'>
              <input
      ...
      <div className='mt-14'>
        <h2 className='text-2xl'>
          <span className='text-slate-800 font-medium'>Kupon</span> Listesi
        </h2>
        <div className='overflow-x-auto mt-4 rounded-lg border border-slate-200 max-w-4xl'>
          <table className='min-w-full bg-white text-sm'>
            <thead className='bg-slate-50'>
              <tr>
                <th className='py-3 px-4 text-left font-semibold text-slate-600'>Kod</th>
                <th className='py-3 px-4 text-left font-semibold text-slate-600'>Açıklama</th>
                <th className='py-3 px-4 text-left font-semibold text-slate-600'>İndirim</th>
                <th className='py-3 px-4 text-left font-semibold text-slate-600'>S.Kull. Tarihi</th>
                <th className='py-3 px-4 text-left font-semibold text-slate-600'>Yeni Kullanıcı</th>
                <th className='py-3 px-4 text-left font-semibold text-slate-600'>Kayıtlı Üye</th>
                <th className='py-3 px-4 text-left font-semibold text-slate-600'>Durum</th>
              </tr>
            </thead>
              ...
              {coupons.map((coupon) => (
                <tr key={coupon.code} className='hover:bg-slate-50'>
                  ...
                  <td className='py-3 px-4 text-slate-800'>
                    <DeleteIcon
                      onClick={() =>
                        toast.promise(deleteCoupon(coupon.code), { loading: 'Kupon siliniyor...' })
                      }
